...
from b_models_and_sub_models.c_sub_models import conv3BnRelu, conv1BnRelu, Attention, BasicBlockDe, SCSA_Block
...
class ReverseStudent(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        out = self.SCSA(out)
        if  self.DG:
            resi1 = self.residualLayer0_1(x[0])#residuallayer是做了卷积调整维度和经历了注意力模块的处理
            resi2 = self.residualLayer1_2(x[1])
            resi3 = self.residualLayer2_3(x[2])

        feature_x = self.layer0(out)
        feature_x = feature_x + resi3 if  self.DG else feature_x
        feature_a = self.layer1(feature_x)
        feature_a = feature_a + resi2 if  self.DG else feature_a
        feature_b = self.layer2(feature_a)
        feature_b = feature_b + resi1 if  self.DG else feature_b
